-- Saves one RecipeVersion and its beans atomically for the authenticated user.
-- Input uses the frontend's camelCase recipe payload. The function returns only
-- the saved identifiers; callers should reload RecipeSeries separately.
drop function if exists public.save_recipe_version(jsonb);

create function public.save_recipe_version(payload jsonb)
returns table (
...
begin
  ...
  select count(*)
  into v_bean_count
  from jsonb_array_elements(v_beans);
  ...
  if exists (
    ...
    where bean.value ? 'beanId'
      ...
      and not exists (
        select 1
        from public.beans
        where id = (bean.value->>'beanId')::uuid
          and user_id = v_user_id
      )
  ) then
  ...
  v_saved_at := case when payload ? 'savedAt' and nullif(payload->>'savedAt', '') is not null then (payload->>'savedAt')::timestamptz else now() end;
  ...
  if v_requested_series_id is null then
    ...
    insert into public.recipe_series (
      user_id,
      name,
      goal,
      status
    ...
    where series.id = v_requested_series_id
  ...
  select
    ...
    v_version_id,
    case
      when nullif(bean.value->>'beanId', '') is null then null
      else (bean.value->>'beanId')::uuid
    end,
...
revoke execute
  on function public.save_recipe_version(jsonb)
  from public;
