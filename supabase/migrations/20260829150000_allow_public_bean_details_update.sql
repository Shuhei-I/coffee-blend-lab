...
begin
  ...
  if new.snapshot_id <> old.snapshot_id
    and current_setting('app.allow_publication_snapshot_replace', true) <> 'on' then
    raise exception 'post snapshot can only be replaced by the publication RPC';
  end if;
...
begin
  ...
  if v_existing_post_id is null then
  ...
  elsif v_existing_include_bean_details <> v_include_bean_details then
    insert into public.published_blend_snapshots (
      owner_user_id,
      source_series_id,
      source_version_id,
    ...
    perform set_config('app.allow_publication_snapshot_replace', 'on', true);
    update public.posts
    set
